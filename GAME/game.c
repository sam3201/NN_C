...
#define WORKER_COUNT 8
    ...
    static int can_afford(const Recipe *r) {
      return inv_wood >= r->wood && inv_stone >= r->stone &&
             inv_gold >= r->gold && inv_food >= r->food;
    }
    ...
    static void make_save_file_path(char *out, size_t cap,
    ...
    static void make_agent_model_path(char *out, size_t cap,
                                      const char *world_name, int agent_id) {
    ...
    static void player_fire_bow_charged(Vector2 dir, float charge01) {
      ...
      int dmg =
          (int)roundf(lerp((float)BOW_DMG_MIN, (float)BOW_DMG_MAX, charge01));
      if (has_sword)
    ...
    static inline float px(float base) {
      return base * WORLD_SCALE * scale_size;
    }
    ...
    static int agent_try_craft_action(Agent *a, Tribe *tr,
                                      ActionType craft_action, float *reward) {
...
#undef TRY_CRAFT
            ...
            for (int dy = -1; dy <= 1 && p->alive; dy++) {
              ...
              for (int mi = 0; mi < MAX_MOBS; mi++) {
                ...
                if (Vector2Distance(p->pos, mw) < (mr * 0.85f)) {
                  ...
                  if (m->health <= 0) {
                    Vector2 mob_world_pos = (Vector2){
                        (float)((cx + dx) * CHUNK_SIZE) + m->position.x,
    ...
    static void agent_try_move(Agent *a, Vector2 dir) {
      ...
      if (world_pos_blocked_nearby(cx, cy, next, agent_radius_world(), -999999,
        ...
        if (!world_pos_blocked_nearby(cxX, cyX, tryX, agent_radius_world(),
                                      -999999, -999999))
          a->position = tryX;
    ...
    static int agent_try_attack_forward(Agent *a, Tribe *tr, float *reward) {
      ...
      if (hit.kind != HIT_MOB) {
        ...
        a->attack_cd =
            agent_attack_cooldown() * 0.35f; // small “whiff” cooldown
    ...
    static int agent_try_harvest_forward(Agent *a, Tribe *tr, float *reward) {
      ...
      if (hit.kind != HIT_RESOURCE) {
        ...
        a->harvest_cd =
            agent_harvest_cooldown() * 0.35f; // small whiff cooldown
    ...
    static void draw_pickups(void) {
      for (int i = 0; i < MAX_PICKUPS; i++) {
        ...
        case PICK_FOOD: {
          ...
          DrawCircleLines((int)sp.x, (int)sp.y, s * 0.75f,
                          (Color){0, 0, 0, 160});
    ...
    */
    ...
    static void draw_health_bar(Vector2 sp, float w, float h, float t01,
      ...
      DrawRectangle((int)(sp.x - w * 0.5f), (int)(sp.y - h), (int)fw, (int)h,
                    fill);
      // outline
      DrawRectangleLines((int)(sp.x - w * 0.5f), (int)(sp.y - h), (int)w,
    ...
    static void init_mob(Mob *m, MobType type, Vector2 local_pos,
                         int make_angry) {
      m->type = type;
    ...
    static void spawn_raid_wave(void) {
      ...
      for (int t = 0; t < TRIBE_COUNT; t++) {
        ...
        for (int k = 0; k < count; k++) {
          ...
          float r =
              tr->base.radius + randf(6.0f, 14.0f); // distance from base edge
    ...
    static int agent_face_nearest_mob_in_chunk(Agent *a, Chunk *c, int cx,
                                               int cy, float maxRange) {
      Vector2 origin =
          (Vector2){(float)(cx * CHUNK_SIZE), (float)(cy * CHUNK_SIZE)};
      float bestD = 1e9f;
      Vector2 bestDir = {0};
    ...
    void update_agent(Agent *a) {
      ...
      if (in_base) {
        ...
        a->stamina =
            fminf(100.0f, a->stamina + (STAMINA_REGEN_RATE * 0.55f) * dt);
      }
    ...
    */
    ...
    static void update_mob_ai(Mob *m, Vector2 chunk_origin, float dt) {
      ...
      if (world_pos_blocked_nearby(mcx, mcy, nextW, mob_radius_world(m->type),
                                   mcx, mcy)) {
    ...
    static void do_world_select_screen(void) {
      ...
      if (ui_button(rPlay,
                    hasSelection ? "Play Selected" : "Play (no world)")) {
        if (hasSelection) {
