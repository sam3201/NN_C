...
(muzero) samueldasari @Samuels - MacBook -
    Air GAME % ls build_game.sh environment.c environment.h game.c(muzero)
                   samueldasari @Samuels -
    MacBook -
    Air GAME % cat *
#!/ bin / bash

        if[!-d "../utils/Raylib"];
then echo "Raylib not found. Installing..." git clone https
    : // github.com/raysan5/raylib.git ../utils/Raylib/
      else echo "Raylib found. Skipping installation." fi
...
gcc -w game.c $MUZE_SRC environment.c \
    -I../utils/Raylib/src \
    -I../utils/NN/MUZE \
...
if [ $? -eq 0 ];
then echo "Compilation successful! Running the game..."./ game else echo
    "Compilation failed!" exit 1 fi

    echo "Cleaning up..." rm game
...
(muzero) samueldasari @Samuels - MacBook - Air GAME %
