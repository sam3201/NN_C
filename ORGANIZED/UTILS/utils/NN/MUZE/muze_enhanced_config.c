...
int validate_enhanced_config(MuzeEnhancedConfig *config) {
    ...
    // Validate action space configuration (only if enabled)
    ...
    // Validate observation configuration (only if enabled)
...
int get_total_discrete_actions(MuzeEnhancedConfig *config) {
    ...
    if (!config->use_discrete_actions) {
        // Continuous actor mode: keep action_count small to avoid enormous output layers.
        return 4;
    }
...
size_t get_compressed_obs_size(MuzeEnhancedConfig *config) {
    ...
    // Return observation dimension in floats (used as model_config->obs_dim).
    // CompressedObs contains fixed-size buffers; we treat the struct as a flat float array.
    return sizeof(CompressedObs) / sizeof(float);
}
