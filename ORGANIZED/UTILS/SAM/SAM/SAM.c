...
SAM_t *SAM_init(size_t input_dim, size_t output_dim, size_t num_heads,
  ...
  size_t transformer_layers = 1; // start simple; can increase later
  sam->transformer = TRANSFORMER_init(input_dim, num_heads, transformer_layers);
...
void SAM_train_submodel(NEAT_t *neat, long double learning_rate) {
  ...
  // Create dummy training data for NEAT evolution
  size_t pop_size = neat->pop->size;
  long double **inputs = malloc(pop_size * sizeof(long double*));
  long double **targets = malloc(pop_size * sizeof(long double*));
  ...
  // Initialize simple training data
  for (size_t i = 0; i < pop_size; i++) {
    inputs[i] = calloc(neat->input_dims, sizeof(long double));
    targets[i] = calloc(neat->output_dims, sizeof(long double));
    
    if (inputs[i] && targets[i]) {
      // Simple pattern: input influences target
      for (size_t j = 0; j < neat->input_dims; j++) {
        inputs[i][j] = (long double)j / neat->input_dims;
      }
      for (size_t j = 0; j < neat->output_dims; j++) {
        targets[i][j] = sin(inputs[i][j % neat->input_dims] * learning_rate);
      }
    }
  }
  
  // Train NEAT with the generated data
  NEAT_train(neat, inputs, targets, pop_size);
...
PerformanceMetrics SAM_calculate_metrics(NEAT_t *neat) {
  ...
  if (neat && neat->pop && neat->pop->genomes && neat->pop->size > 0) {
    // Calculate average fitness across the population
    long double total_fitness = 0.0L;
    long double best_fitness = -1.0L;
    size_t valid_genomes = 0;
    
    for (size_t i = 0; i < neat->pop->size; i++) {
      if (neat->pop->genomes[i]) {
        // Use a simple fitness calculation based on genome complexity
        long double genome_fitness = (long double)neat->pop->genomes[i]->numConnections / 
                                   (neat->pop->genomes[i]->numNodes + 1);
    ...
    if (valid_genomes > 0) {
      metrics.fitness = total_fitness / valid_genomes;
      metrics.accuracy = best_fitness; // Use best fitness as accuracy proxy
      metrics.loss = 1.0L / (1.0L + metrics.fitness); // Better loss calculation
    }
  }
...
void SAM_project(SAM_t *sam, long double **source_data, size_t data_length) {
  if (!sam || !source_data || data_length == 0)
    return;
  
  // Create projection matrix for transformer → submodel knowledge transfer
  ProjectionMatrix *P = SAM_create_projection_matrix(sam, sam->context);
  if (!P)
    return;
  
  // For each submodel, project transformer knowledge
  for (size_t i = 0; i < sam->num_submodels; i++) {
    if (sam->submodels && sam->submodels[i]) {
      // Calculate context-aware scaling factor
      long double gamma = SAM_calculate_gamma(sam->context, i);
      
      // Apply projection to transfer knowledge
      // This is a simplified implementation - in practice would involve
      // actual weight matrix operations between transformer and NEAT
      AdaptationParams params;
      params.learning_rate_transfusion = gamma * 0.01L;
      params.learning_rate_generalization = 0.001L;
      
      SAM_adapt_transfusion(sam, sam->context, P, &params);
    }
  }
  
  // Cleanup projection matrix
  if (P->matrix) {
    for (size_t i = 0; i < P->rows; i++) {
      free(P->matrix[i]);
    }
