#include "FORERUNNER.h"
#include "../utils/NN/TRANSFORMER/TRANSFORMER.h"
#include <math.h>

// Initialize the ForeRunner model
ForeRunner_t *ForeRunner_init(size_t input_dim, size_t num_heads,
                              unsigned int num_contents) {
  ForeRunner_t *model = (ForeRunner_t *)malloc(sizeof(ForeRunner_t));
  if (!model)
    return NULL;

  model->transformer = TRANSFORMER_init(input_dim, num_heads, num_contents);
  if (!model->transformer) {
    free(model);
    return NULL;
  }

  model->num_contexts = num_contents;

  return model;
}

// Compute context from input data
unsigned int ForeRunner_forward(ForeRunner_t *model,
                                long double **input_sequence,
                                unsigned int seq_length) {
  if (!model || !input_sequence || seq_length == 0)
    return NONE;

  long double **out =
      TRANSFORMER_forward(model->transformer, input_sequence, seq_length);
  if (!out)
    return NONE;

  unsigned int C = model->num_contexts;
  unsigned int best = 0;
  long double bestv = out[seq_length - 1][0];

  for (unsigned int i = 1; i < C; i++) {
    if (out[seq_length - 1][i] > bestv) {
      bestv = out[seq_length - 1][i];
      best = i;
    }
  }

  // free output sequence
  for (size_t t = 0; t < seq_length; t++)
    free(out[t]);
  free(out);

  return best;
}

// Backpropagation function with gradient computation
void ForeRunner_backprop(ForeRunner_t *model, long double **input_sequence,
                         unsigned int seq_length, long double *grad_loss) {
  if (!model || !input_sequence || seq_length == 0 || !grad_loss)
    return;
  
  // Build gradient output for transformer backpropagation
  size_t model_dim = model->transformer->input_dim;
  long double **grad_output = malloc(seq_length * sizeof(long double*));
  if (!grad_output)
    return;
  
  for (unsigned int t = 0; t < seq_length; t++) {
    grad_output[t] = malloc(model_dim * sizeof(long double));
    if (!grad_output[t]) {
      // Cleanup allocated memory
      for (unsigned int i = 0; i < t; i++) {
        free(grad_output[i]);
      }
      free(grad_output);
      return;
    }
    
    // Initialize gradients - for context classification, we only care about final timestep
    if (t == seq_length - 1) {
      // Use the provided gradient loss for the final timestep
      for (size_t i = 0; i < model_dim; i++) {
        grad_output[t][i] = (i < model->num_contexts) ? grad_loss[i] : 0.0L;
      }
    } else {
      // Zero gradients for other timesteps
      for (size_t i = 0; i < model_dim; i++) {
        grad_output[t][i] = 0.0L;
      }
    }
  }
  
  // Call transformer backpropagation
  TRANSFORMER_backprop(model->transformer, input_sequence, seq_length, grad_output);
  
  // Cleanup gradient output
  for (unsigned int t = 0; t < seq_length; t++) {
    free(grad_output[t]);
  }
  free(grad_output);
}

// Destroy the ForeRunner model
void ForeRunner_destroy(ForeRunner_t *model) {
  if (!model)
    return;

  if (model->transformer) {
    TRANSFORMER_destroy(model->transformer);
  }

  free(model);
}
