...
TrainingData *load_all_text_files(const char *dir_path,
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    while (fgets(line, sizeof(line), f)) {
      ...
      if (word_count < sequence_length + 1)
        continue;
      ...
      for (size_t i = 0; i <= word_count - sequence_length - 1; i++) {
        if (data->num_samples >= capacity) {
          capacity *= 2;
          data->sequences =
              (char ***)realloc(data->sequences, capacity * sizeof(char **));
          data->targets =
              (char **)realloc(data->targets, capacity * sizeof(char *));
        }

        // Allocate sequence
        data->sequences[data->num_samples] =
            (char **)malloc(sequence_length * sizeof(char *));
...
void encode_sequence(char **words, size_t num_words, long double *encoded,
                     size_t dim) {
  memset(encoded, 0, dim * sizeof(long double));
...
int main(int argc, char *argv[]) {
  ...
  if (test_data && test_data->num_samples > 0) {
    ...
    if (train_data) {
      ...
      train_data->sequences =
          (char ***)realloc(train_data->sequences, new_count * sizeof(char **));
      train_data->targets =
          (char **)realloc(train_data->targets, new_count * sizeof(char *));
    } else {
    ...
    // Free test_data structure (but not the actual data, it's now in
    // train_data)
  ...
  for (size_t epoch = 0; epoch < num_epochs; epoch++) {
    ...
    for (size_t i = 0; i < train_data->num_samples; i++) {
      ...
      long double *input_vec =
          (long double *)calloc(MODEL_DIM, sizeof(long double));
      encode_sequence(train_data->sequences[i], SEQUENCE_LENGTH, input_vec,
                      MODEL_DIM);

      // Encode target
      long double *target =
          (long double *)calloc(OUTPUT_DIM, sizeof(long double));
  ...
  snprintf(filename_timestamped, sizeof(filename_timestamped),
           "../sam_chatbot_%s.bin", timestamp);
