...
void decode_action_from_vector(long double *vector, HybridAction *action, int vector_dim) {
    ...
    action->discrete_action = max_idx;
...
long double* vision_expert_forward(Context *ctx) {
    VisionExpert *expert = (VisionExpert*)ctx;
...
long double* combat_expert_forward(Context *ctx) {
    CombatExpert *expert = (CombatExpert*)ctx;
...
long double* navigation_expert_forward(Context *ctx) {
    NavigationExpert *expert = (NavigationExpert*)ctx;
...
long double* physics_expert_forward(Context *ctx) {
    PhysicsExpert *expert = (PhysicsExpert*)ctx;
...
// Softmax function
void softmax(long double *values, int count) {
    ...
    for (int i = 0; i < count; i++) {
        values[i] = exp(values[i] - max_val) / sum;
    }
}
...
void route_experts(HeadModule *head, Context *ctx, long double *routing_weights) {
    ...
    softmax(routing_weights, head->expert_count);
...
void fuse_experts(HeadModule *head, long double **expert_outputs, long double *fused_weights, long double *fused_weights_stds, int expert_count, int expert_dim) {
    ...
    for (int i = 0; i < expert_dim; i++) {
        ...
        for (int j = 0; j < expert_count; j++) {
            fused_weights[i] += used_weights[j] * expert_outputs[j][i];
        }
        used_weights_stds[i] = sqrt(fused_weights[i]);
    }
}
...
void generate_discrete_policy(HeadModule *head, long double *fused_weights, long double *discrete_logits) {
    ...
    for (int i = 0; i < DISCRETE_ACTIONS; i++) {
        ...
        for (int j = 0; j < 64; j++) {
            discrete_logits[i] += head->discrete_weights[i * DISCRETE_ACTIONS + j] * used_weights[j];
        }
    }
    softmax(discrete_logits, DISCRETE_ACTIONS);
}
...
void generate_continuous_policy(HeadModule *head, long double *fused_weights, long double *continuous_means, long double *continuous_stds, HybridAction *action) {
    ...
    for (int i = 0; i < CONTINUOUS_ACTIONS; i++) {
        ...
        for (int j = 0; j < 64; j++) {
            continuous_means[i] += head->continuous_weights[i * CONTINUOUS_ACTIONS + j] * used_weights[j];
        }
...
void generate_hybrid_action(HeadModule *head, Context *ctx, HybridAction *action) {
    // Route experts
    route_experts(head, ctx, head->routing_weights);
    
    // Get expert outputs (simplified)
    long double **expert_outputs = malloc(head->expert_count * sizeof(long double*));
    long double *used_weights = malloc(head->expert_count * sizeof(long double));
    long double *used_weights_stds = malloc(head->expert_count * sizeof(long double));
    
    for (int i = 0; i < head->expert_count; i++) {
        expert_outputs[i] = malloc(64 * sizeof(long double));
        
        // Simulate expert forward pass
        if (strcmp(head->routing_weights[i], "Vision") == 0) {
            VisionExpert *expert = init_vision_expert(64);
            ...
            free(output);
            free(expert);
        } else if (strcmp(head->routing_weights[i], "Combat") == 0) {
            CombatExpert *expert = init_combat_expert(64);
            ...
            free(output);
            free(expert);
        } else if (strcmp(head->routing_weights[i], "Navigation") == 0) {
            NavigationExpert *expert = init_navigation_expert(64);
            ...
            free(output);
            free(expert);
        } else if (strcmp(head->routing_weights[i], "Physics") == 0) {
            PhysicsExpert *expert = init_physics_expert(64);
        ...
        used_weights[i] = head->routing_weights[i];
        used_weights_stds[i] = 1.0L;
    }
    
    // Fuse expert outputs
    fuse_experts(head, expert_outputs, used_weights, used_weights_stds, head->expert_count, 64);
    
    // Generate policies
    generate_discrete_policy(head, used_weights, action->action_vector);
    generate_continuous_policy(head, used_weights, continuous_means, continuous_stds, action);
    
    // Decode action from vector
    decode_action_from_vector(action->action_vector, action, CONTEXT_DIM);
...
void world_model_forward(WorldModel *model, Context *ctx, HybridAction *action, long double *next_state, long double *reward, int *done) {
    ...
    for (int i = 0; i < model->context_dim; i++) {
        next_state[i] = 0.0L;
        for (int j = 0; j < model->latent_dim; j++) {
            next_state[i] += model->encoder_weights[i * model->latent_dim + j] * ctx->context_vector[j];
        }
    }
    ...
    for (int i = 0; i < model->latent_dim; i++) {
        next_state[i] = tanh(next_state[i]); // Activation
    }
...
void sample_hybrid_action(HeadModule *head, Context *ctx, HybridAction *action) {
    ...
    if ((double)rand() / RAND_MAX < 0.1) {
        ...
        // Re-encode
        encode_hybrid_action(action, action->action_vector, CONTEXT_DIM);
    }
}
...
void test_hybrid_actions() {
    ...
    printf("Mode: %s\n", get_action_name(test_action.action_mode));
    printf("Discrete action: %d\n", test_action.discrete_action);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        printf("Sample %d: %s (%d, ", get_action_name(test_action.action_mode));
        printf("Discrete: %d) - Continuous: ");
    ...
    world_model_forward(world, ctx, &test_action, next_state, &reward, &done);
    ...
    if (best_node) {
        ...
        printf("  - Best action: %s (%d)\n", get_action_name(best_node->action.action_mode), best_node->discrete_action);
        printf("  - Continuous actions: ");
        for (int i = 0; i < CONTINUOUS_ACTIONS; i++) {
