...
typedef struct MCTSNode {
    ...
    int parent_index;
    int child_count;
    int children_indices[MAX_PLANNING_DEPTH * DISCRETE_ACTIONS * 10]; // Simplified
...
// MCTS tree
typedef struct {
    MCTSNode nodes[MAX_PLANNING_DEPTH * DISCRETE_ACTIONS * 10 + 1]; // +1 for root
    int node_count;
    int root_index;
    int current_depth;
...
typedef struct {
    ...
    long double planner_value;
    long double expert_features[4][32];
    long double core_features[32];
...
typedef struct {
    ...
    long double policy_target[DISCRETE_ACTIONS];
} TrainingSample;
...
void init_mcts_tree(MCTSTree *tree) {
    ...
    for (int i = 0; i < MAX_PLANNING_DEPTH * DISCRETE_ACTIONS * 10; i++) {
...
int add_child_node(MCTSTree *tree, int parent_index, Context *context, HybridAction *action) {
    ...
    // Update parent
    parent->children_indices[parent->child_count] = child_index;
    parent->child_count++;
...
void expand_node(MCTSTree *tree, int parent_index, HeadModule *head) {
    MCTSNode *parent = &tree->nodes[parent_index];
    ...
    for (int discrete_action = 0; discrete_action < DISCRETE_ACTIONS; discrete_action++) {
        for (int continuous_sample = 0; continuous_sample < 3; continuous_sample++) {
            ...
            if (child_index != -1) {
                ...
                long double reward;
                WorldModel world_model = world; // Use the passed world model
                world_model_forward(world_model, &child->context, &action, next_state, &reward);
                
                // Simple value estimation
                child->policy_value = reward;
                child->action_probability = 0.1L; // Equal probability for now
...
// MCTS search
void mcts_search(MCTSTree *tree, HeadModule *head, WorldModel *world, int simulations) {
    for (int sim = 0; sim < simulations; sim++) {
        // Selection
        int current_index = tree->root_index;
        while (tree->nodes[current_index].child_count > 0 && !tree->nodes[current_index].is_terminal) {
            current_index = select_best_child(tree, current_index, head, world);
        }
        
        // Expansion
        if (tree->nodes[current_index].child_count == 0 && !tree->nodes[current_index].is_terminal) {
            expand_node(tree, current_index, head);
        }
        
        // Simulation
        int leaf_index = current_index;
        if (tree->nodes[current_index].child_count > 0) {
            leaf_index = tree->nodes[current_index].children_indices[0];
        }
        simulate_node(tree, leaf_index, head, world);
        
        // Backpropagation
        backpropagate(tree, leaf_index, tree->nodes[leaf_index].value);
    }
}

// Get best action from MCTS
HybridAction get_best_action(MCTSTree *tree) {
    MCTSNode *root = &tree->nodes[tree->root_index];
    
    if (root->child_count == 0) {
        // No children, return default action
        HybridAction action;
        action.discrete_action = IDLE;
        action.action_mode = MODE_ON_FOOT;
        for (int i = 0; i < CONTINUOUS_ACTIONS; i++) {
            action.continuous_actions[i] = 0.0L;
        }
        encode_hybrid_action(&action);
        return action;
    }
    ...
    if (best_child_index == -1) {
        ...
        best_child_index = root->children_indices[0];
    }
    
    return tree->nodes[best_child_index].action;
}

// Initialize transfusion data
void init_transfusion_data(TransfusionData *data) {
    for (int i = 0; i < DISCRETETE_ACTIONS; i++) {
...
void transfuse_experts_to_core(TransfusionData *data, HeadModule *head, ExpertOutputs *outputs) {
    ...
    for (int j = 0; j < 32; j++) {
        long double expert_avg = 0.0L;
        expert_avg += (data->expert_features[0][j] + data->expert_features[1][j] + data->expert_features[2][j] + data->expert_features[3][j]) / 4.0L;
...
void sample_training_data(HeadModule *head, WorldModel *world, TrainingSample *sample) {
    ...
    if (!sample->done) {
        sample->next_value += 0.99L * 0.1L; // Simple value estimation
    }
