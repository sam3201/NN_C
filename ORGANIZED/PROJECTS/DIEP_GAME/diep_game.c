...
// Simple AI functions (placeholder for MUZE)
...
int main(int argc, char *argv[]) {
    ...
    // Create window
    game.window = SDL_CreateWindow("Diep.io Tank Game", 
...
void init_simple_ai(GameState *game) {
    printf("Simple AI System Initialized (MUZE integration pending)\n");
}
...
void update_game(GameState *game) {
    ...
    if (game->player.alive) {
        ...
        game->player.x = fmaxf(ARENA_MARGIN + game->player.radius, 
                              fminf(SCREEN_WIDTH - ARENA_MARGIN - game->player.radius, game->player.x));
        game->player.y = fmaxf(ARENA_MARGIN + game->player.radius, 
                              fminf(SCREEN_HEIGHT - ARENA_MARGIN - game->player.radius, game->player.y));
    }
    ...
    if (game->ai_tank.alive) {
        ...
        // Keep AI in screen bounds
        game->ai_tank.x = fmaxf(ARENA_MARGIN + game->ai_tank.radius, 
                               fminf(SCREEN_WIDTH - ARENA_MARGIN - game->ai_tank.radius, game->ai_tank.x));
        game->ai_tank.y = fmaxf(ARENA_MARGIN + game->ai_tank.radius, 
                               fminf(SCREEN_HEIGHT - ARENA_MARGIN - game->ai_tank.radius, game->ai_tank.y));
    }
...
void update_simple_ai(GameState *game) {
    ...
    if (fabs(angle_diff) > TANK_ROTATION_SPEED) {
        game->ai_tank.angle += (angle_diff > 0) ? TANK_ROTATION_SPEED : -TANK_ROTATION_SPEED;
    } else {
        game->ai_tank.angle = target_angle;
    }
    
    // Move towards player if far, move away if close
    if (distance > 200) {
        game->ai_tank.vx = cosf(game->ai_tank.angle) * TANK_SPEED * 0.8f;
        game->ai_tank.vy = sinf(game->ai_tank.angle) * TANK_SPEED * 0.8f;
    } else if (distance < 150) {
        game->ai_tank.vx = -cosf(game->ai_tank.angle) * TANK_SPEED * 0.5f;
        game->ai_tank.vy = -sinf(game->ai_tank.angle) * TANK_SPEED * 0.5f;
    } else {
        game->ai_tank.vx *= 0.9f;
        game->ai_tank.vy *= 0.9f;
    }
    ...
    if (fabs(angle_diff) < 0.2f && distance < 400) {
...
void render_game(GameState *game) {
    ...
    SDL_Rect arena = {ARENA_MARGIN, ARENA_MARGIN, 
    ...
    if (game->paused) {
        ...
        for (int x = 0; x <= GRID_WIDTH; x++) {
            int screen_x = ARENA_MARGIN + x * cell_width;
            SDL_RenderDrawLine(game->renderer, screen_x, ARENA_MARGIN, 
                             screen_x, SCREEN_HEIGHT - ARENA_MARGIN);
        }
        for (int y = 0; y <= GRID_HEIGHT; y++) {
            int screen_y = ARENA_MARGIN + y * cell_height;
            SDL_RenderDrawLine(game->renderer, ARENA_MARGIN, screen_y, 
                             SCREEN_WIDTH - ARENA_MARGIN, screen_y);
        }
    }
...
void fire_bullet(GameState *game, int owner) {
    ...
    if (!tank->alive || current_time - game->last_fire_time[owner] < FIRE_RATE) {
        return;
    }
    ...
    for (int i = 0; i < MAX_BULLETS; i++) {
        if (!game->bullets[i].active) {
            ...
            game->bullets[i].color = tank->color;
            game->bullets[i].active = true;
            game->bullets[i].owner = owner;
            game->bullets[i].entity_type = (owner == 0) ? ENTITY_BULLET_SELF : ENTITY_BULLET_ENEMY;
    ...
    for (int i = 0; i < MAX_BULLETS; i++) {
        if (game->bullets[i].active) {
            ...
            if (game->bullets[i].x < ARENA_MARGIN || 
                game->bullets[i].x > SCREEN_WIDTH - ARENA_MARGIN ||
                game->bullets[i].y < ARENA_MARGIN || 
                game->bullets[i].y > SCREEN_HEIGHT - ARENA_MARGIN) {
                game->bullets[i].active = false;
            }
        }
    }
...
void add_entity_to_grid(GameState *game, float x, float y, float entity_type) {
    ...
    int grid_x = (int)((x - ARENA_MARGIN) / (SCREEN_WIDTH - 2 * ARENA_MARGIN) * GRID_WIDTH);
    int grid_y = (int)((y - ARENA_MARGIN) / (SCREEN_HEIGHT - 2 * ARENA_MARGIN) * GRID_HEIGHT);
...
void draw_tank(SDL_Renderer *renderer, Tank *tank) {
    // Draw tank body
    draw_circle(renderer, (int)tank->x, (int)tank->y, (int)tank->radius, tank->color);
    ...
    float cannon_length = tank->radius + 15;
    float cannon_end_x = tank->x + cosf(tank->angle) * cannon_length;
    float cannon_end_y = tank->y + sinf(tank->angle) * cannon_length;
    
    SDL_RenderDrawLine(renderer, (int)tank->x, (int)tank->y, 
                     (int)cannon_end_x, (int)cannon_end_y);
}

void draw_bullet(SDL_Renderer *renderer, Bullet *bullet) {
    draw_circle(renderer, (int)bullet->x, (int)bullet->y, 
              (int)bullet->radius, bullet->color);
}
...
void draw_ui(SDL_Renderer *renderer, TTF_Font *font, GameState *game) {
    ...
    // Draw controls
    char controls_text[] = "Controls: WASD=Move, Space=Shoot, P=Pause (shows grid)";
