...
void level_up(Agent* agent) {
    while (agent->total_xp >= agent->level * XP_PER_LEVEL) {
        agent->total_xp -= agent->level * XP_PER_LEVEL;
        agent->level++;
...
void gain_xp_from_food(Agent* agent) {
    agent->total_xp += 1;  // Each food gives 1 XP
    level_up(agent);
}

void gain_xp_from_agent(Agent* agent, Agent* eaten_agent) {
    agent->total_xp += eaten_agent->total_xp;
    agent->num_eaten++;
    level_up(agent);
    update_agent_color(agent);
}
...
void gain_xp_from_offspring(Agent* agent) {
    agent->total_xp += agent->level * 10;  // Bonus XP for successful breeding
...
void transfer_weights_with_mutation(NN_t* old_nn, NN_t* new_nn, int level) {
    ...
    for (size_t i = 0; i < old_nn->numLayers - 1; i++) {
        size_t weights_size = old_nn->layers[i] * old_nn->layers[i + 1];
...
void spawn_food(Food* food) {
    food->position.x = (float)(rand() % SCREEN_WIDTH);
    food->position.y = (float)(rand() % SCREEN_HEIGHT);
    food->rect = (Rectangle){
        food->position.x,
        food->position.y,
        1,  // width
...
void execute_action(GameState* game, int agent_idx, Action action) {
    Agent* agent = &game->agents[agent_idx];
    float speed = 1.0f;  // Base movement speed
...
void eat_agent(Agent* predator, Agent* prey) {
    predator->total_xp += prey->total_xp;
    predator->num_eaten++;
}
