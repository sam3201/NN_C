...
then echo "Raylib not found. Installing..." git clone https
    : // github.com/raysan5/raylib.git "$RAYLIB_DIR"
      else echo "Raylib found. Skipping installation." fi

      if[!-f "$RAYLIB_LIB"];
then echo
    "Building Raylib..."(cd "$RAYLIB_SRC" &&make PLATFORM = PLATFORM_DESKTOP) fi
...
#Trim leading / trailing spaces(prevents phantom args)
    MUZE_SRC = "$(printf " % s " " $MUZE_SRC
                               " | sed 's/^ *//; s/ *$//')" SAM_SRC =
                   "$(printf " % s " " $SAM_SRC " | sed 's/^ *//; s/ *$//')"

                   if[-z "$MUZE_SRC"];
then echo "ERROR: No MUZE .c files found under ../utils/NN/MUZE" exit 1 fi

    if[-z "$SAM_SRC"];
then echo "ERROR: No SAM .c files found under ../SAM" exit 1 fi

    echo "MUZE files:" printf "  %s\n" $MUZE_SRC echo "SAM files:" printf
         "  %s\n" $SAM_SRC
