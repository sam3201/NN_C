...
void update_agent_color(Agent *agent) {
  float xp_factor =
      (float)agent->total_xp / (float)(agent->level * XP_PER_LEVEL);
...
void evolve_population(GameState *game) {
  ...
  for (int i = 0; i < POPULATION_SIZE; i++) {
    if (game->agents[i].level >= 0) {
      game->agents[i].brain->nodes[0]->fitness =
          calculate_fitness(&game->agents[i]);
    }
  }
...
void handle_breeding_completion(GameState *game, int agent_idx) {
  ...
  if (partner_idx >= 0) {
    ...
    if (offspring_idx >= 0) {
      ...
      offspring->brain =
          NEAT_crossover(parent1->brain->nodes[0], parent2->brain->nodes[0]);
...
void evolve_agent(Agent *agent) {
  ...
  unsigned int maturity_bonus =
      (unsigned int)(agent->time_alive /
                     60.0f); // Bonus neurons based on survival time
  unsigned int level_bonus = agent->level * 4; // Bonus neurons based on level
  unsigned int experience_bonus = (agent->num_eaten + agent->num_offsprings) *
                                  2; // Bonus based on experiences

  // Calculate total neurons for each hidden layer
  unsigned int total_neurons =
      base_neurons + maturity_bonus + level_bonus + experience_bonus;
  ...
  ActivationDerivativeType actDerivs[] = {
      ...
      agent->level > 5 ? SIGMOID_DERIVATIVE : RELU_DERIVATIVE, RELU_DERIVATIVE,
      SIGMOID_DERIVATIVE};

  // Adjust learning parameters based on maturity
  long double base_learning_rate = 0.01L;
  long double maturity_factor =
      1.0L / (1.0L + agent->time_alive / 3600.0f); // Decreases with age
  long double experience_factor =
      1.0L / (1.0L + (agent->num_eaten + agent->num_offsprings) * 0.1L);
  long double learning_rate =
      base_learning_rate * maturity_factor * experience_factor;

  // Create new neural network with enhanced configuration
  NN_t *new_nn = NN_init(
      layers, actFuncs, actDerivs,
      agent->level > 10
          ? CE
          : (agent->level > 5
                 ? HUBER
                 : MSE), // Loss function complexity increases with level
      agent->level > 10
          ? CE_DERIVATIVE
          : (agent->level > 5 ? HUBER_DERIVATIVE : MSE_DERIVATIVE),
      agent->level > 8 ? L2
                       : L1, // Switch to L2 regularization at higher levels
      agent->level > 12
          ? ADAM
          : (agent->level > 6
                 ? RMSPROP
                 : SGD), // Optimizer complexity increases with level
      learning_rate);
  ...
  Perceptron_t *new_brain =
      Perceptron_init(new_nn, old_brain->fitness, 0, NULL, true);
  if (!new_brain) {
  ...
  printf("Agent %d evolved! Level: %d, Neurons: %u, Time Alive: %.2f, Eaten: "
         "%d, Offspring: %d\n",
...
void transfer_weights_with_mutation(NN_t *old_nn, NN_t *new_nn, int level) {
  ...
  for (size_t i = 0; i < old_nn->numLayers - 1; i++) {
    size_t weights_size =
        old_nn->layers[i] *
        old_nn->layers[i + 1]; // Using layerSizes instead of layers
    for (size_t j = 0; j < weights_size; j++) {
      ...
      if ((float)rand() / RAND_MAX < mutation_chance) {
        // Apply mutation
        new_nn->weights[i][j] =
            old_nn->weights[i][j] + ((float)rand() / RAND_MAX - 0.5f) * 0.2f;
      } else {
        // Direct copy
...
void execute_action(GameState *game, int agent_idx, Action action) {
  ...
  if (agent->rect.x <= 0) {
    ...
    agent->rect.x =
        SCREEN_WIDTH - agent->rect.width - speed; // Move away from right wall
  }

  if (agent->rect.y <= 0) {
    agent->rect.y = speed; // Move away from top wall
  } else if (agent->rect.y + agent->rect.height >= SCREEN_HEIGHT) {
    agent->rect.y = SCREEN_HEIGHT - agent->rect.height -
                    speed; // Move away from bottom wall
  }
...
void handle_breeding(GameState *game, int agent_idx) {
  ...
  for (int i = 0; i < POPULATION_SIZE; i++) {
    if (game->agents[i].level < 0) { // Found empty slot
      ...
      transfer_weights_with_mutation(old_brain->nodes[0]->nn,
                                     new_brain->nodes[0]->nn, parent->level);
      break;
    }
  }
}
