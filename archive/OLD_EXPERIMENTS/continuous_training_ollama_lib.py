#!/usr/bin/env python3
"""
Continuous Training System with Ollama Python Library
Uses official Ollama Python library for better integration
"""
...
try:
    ...
    print("📋 For more info: https://github.com/ollama/ollama-python")
    sys.exit(1)
...
class OllamaIntegration:
    def __init__(self, model_name="llama2"):
        ...
        try:
            ...
            self.chat_log.add_entry("SYSTEM", f"Connected to Ollama client", False)
            ...
            self.chat_log.add_entry("SYSTEM", f"Failed to initialize Ollama: {e}", True)
            raise
    ...
    def _ensure_model_available(self):
        ...
        try:
            ...
            if self.model_name not in model_names:
                ...
                # Pull the model
                result = self.client.pull(self.model_name)
    ...
    def _load_model(self):
        """Load the model"""
        try:
            self.chat_log.add_entry("SYSTEM", f"Loading model {self.model_name}...", False)
            self.model = self.client.chat(model=self.model_name)
            self.chat_log.add_entry("SYSTEM", f"Model {self.model_name} loaded successfully", False)
        except Exception as e:
            self.chat_log.add_entry("SYSTEM", f"Failed to load model: {e}", True)
            raise
    ...
    def generate_response(self, prompt, max_length=500, timeout=30):
        ...
        try:
            ...
            if response and 'message' in response:
                ...
                self.chat_log.add_entry("OLLAMA", content, False)
                return content
            else:
                self.chat_log.add_entry("OLLAMA", "No response generated", True)
                return None
...
class TrainingSession:
    def __init__(self, ollama_model="llama2", training_interval=30):
        ...
        # Initialize Ollama
        try:
            self.ollama = OllamaIntegration(ollama_model)
        except Exception as e:
    ...
    def setup_logging(self):
        ...
        log_filename = f"continuous_training_ollama_lib_{int(time.time())}.log"
        ...
        self.logger = logging.getLogger('continuous_training_ollama_lib')
        self.logger.info("Ollama library continuous training session started")
    ...
    def save_checkpoint(self):
        ...
        checkpoint_name = f"continuous_training_ollama_lib_epoch_{self.epoch_count}.json"
...
class SimpleTerminalInterface:
    ...
    def run(self):
        ...
        print("\n" + "="*60)
        print("🎓 OLLAMA LIBRARY CONTINUOUS TRAINING")
        print("="*60)
        print("Using official Ollama Python library")
    ...
    def display_help(self):
        ...
        print("  🤖 Ollama library integration")
...
def main():
    """Main function"""
    print("=== OLLAMA LIBRARY CONTINUOUS TRAINING ===")
    print("Using official Ollama Python library for better integration")
    print("========================================================\n")
    ...
    session.stop_training_thread_thread()
    ...
    print("\n🎯 OLLAMA LIBRARY INTEGRATION RESULTS:")
