#!/usr/bin/env python3
"""
Threaded Continuous Training System with NCurses Interface
Uses Ollama to teach SAM model with real-time monitoring
"""
...
class TrainingSession:
    ...
    def setup_logging(self):
        ...
        log_filename = f"continuous_training_threaded_{int(time.time())}.log"
        ...
        self.logger = logging.getLogger('continuous_training_threaded')
        self.logger.info("Threaded continuous training session started")
    ...
    def generate_ollama_response_teaching(self, prompt, max_length=500):
        """Generate teaching-focused response using Ollama"""
        ...
        try:
            cmd = ['ollama', 'run', self.ollama_model, enhanced_prompt]
    ...
    def train_sam_model_teaching(self):
        ...
        for i, (input_text, prompt) in enumerate(teaching_prompts):
            ...
            target = self.generate_ollama_response_teaching(prompt)
            ...
            if target:
                ...
                try:
                    ...
                    self.chat_log.add_entry("SAM", loss_msg, loss > 1.0)
...
class NCursesInterface:
    ...
    def create_windows(self):
        ...
        status_height = 3
        log_height = height - chat_height - status_height - 2
        
        # Create windows
        self.chat_window = curses.newwin(chat_height, width, 0, 0)
        self.status_window = curses.newwin(status_height, width, chat_height, 0)
        self.log_window = curses.newwin(log_height, width, chat_height + status_height, 0)
    ...
    def draw_window_borders(self):
        ...
        self.chat_window.addstr(0, 2, " CHAT LOG ")
        ...
        self.log_window.addstr(0, 2, " DEBUG LOG ")
        
    def display_chat_log(self):
        """Display chat log"""
        self.chat_window.erase()
        self.draw_window_borders()
        
        height, width = self.chat_window.getmaxyx()
        
        # Get chat entries
        chat_entries = self.session.chat_log.get_recent_entries(
            count=height-2, 
            entry_types=['USER', 'OLLAMA', 'SAM']
        )
        
        # Display entries (newest first)
        for i, entry in enumerate(reversed(chat_entries)):
            if i >= height - 2:
                break
            ...
            if entry['type'] == 'USER':
                ...
                color = curses.color_pair(7)  # Magenta
            elif entry['type'] == 'SAM':
            ...
            # Display entry
            content = entry['content'][:width-20]  # Truncate if too long
            self.chat_window.addstr(
                height - 2 - i, 2, 
                f"[{timestamp}] {entry['type']}: {content}",
                color
            )
    ...
    def display_status(self):
        ...
        status_text = self.session.get_status_text()
        
        # Display status
        self.status_window.addstr(1, 2, status_text, curses.color_pair(8))
    ...
    def display_debug_log(self):
        ...
        log_entries = self.session.chat_log.get_recent_entries(count=height-2)
    ...
    def handle_input(self):
        ...
        try:
            ...
            if ch == ord('q') or ch == ord('Q'):
                ...
                self.session.chat_log.add_entry("SYSTEM", "Commands: Q-Quit, S-Status, C-Clear log, H-Help", False)
    ...
    def run(self):
        ...
        try:
            ...
            self.session.chat_log.add_entry("SYSTEM", "NCurses interface initialized", False)
            self.session.chat_log.add_entry("SYSTEM", "Commands: Q-Quit, S-Status, C-Clear log, H-Help", False)
...
def main():
    """Main function"""
    print("=== THREADED CONTINUOUS TRAINING WITH NCURSES ===")
    print("Using Ollama to teach SAM model with real-time monitoring")
    print("========================================================\n")
    ...
    interface = NCursesInterface(session)
    interface.run()
