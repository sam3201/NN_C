...
static const ActivationFunction ACTIVATION_FUNCTIONS[ACTIVATION_TYPE_COUNT] = {
    sigmoid,
    tanh_activation,
    relu,
    linear
...
static const ActivationDerivative ACTIVATION_DERIVATIVES[ACTIVATION_DERIVATIVE_TYPE_COUNT] = {
    sigmoid_derivative,
    tanh_derivative,
    relu_derivative,
    linear_derivative
...
const LossFunction LOSS_FUNCTIONS[LOSS_TYPE_COUNT] = {
    mse,
    mae,
    h,
    ll,
...
const LossDerivative LOSS_DERIVATIVES[LOSS_DERIVATIVE_TYPE_COUNT] = {
    mse_derivative,
    mae_derivative,
    h_derivative,
    ll_derivative,
...
static const RegularizationFunction REGULARIZATION_FUNCTIONS[REGULARIZATION_TYPE_COUNT] = {
    l1,
    l2
};

static const OptimizerFunction OPTIMIZER_FUNCTIONS[OPTIMIZER_TYPE_COUNT] = {
    sgd,
    rmsprop,
    adagrad,
    adam,
...
NN_t* NN_init(size_t* layers,
    ...
    printf("NN_init: Starting initialization\n");
    fflush(stdout);
    ...
    // Count layers until we hit 0
    size_t count = 0;
    while (layers[count] != 0) {
        printf("NN_init: Layer %zu has %zu neurons\n", count, layers[count]);
        count++;
    }
    
    printf("NN_init: Found %zu layers\n", count);
    fflush(stdout);
    ...
    nn->numLayers = count;
    if (nn->numLayers < 2) {  // Need at least input and output layer
        free(nn);
        return NULL;
    }
    ...
    nn->weights_gradients = (long double**)malloc((nn->numLayers - 1) * sizeof(long double*));
    nn->biases_gradients = (long double**)malloc((nn->numLayers - 1) * sizeof(long double*));
    nn->weights_m = (long double**)malloc((nn->numLayers - 1) * sizeof(long double*));
    nn->weights_v = (long double**)malloc((nn->numLayers - 1) * sizeof(long double*));
    nn->biases_m = (long double**)malloc((nn->numLayers - 1) * sizeof(long double*));
    nn->biases_v = (long double**)malloc((nn->numLayers - 1) * sizeof(long double*));
    
    // Allocate activation functions and derivatives
    ...
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        size_t current_size = layers[i] * layers[i + 1];
        
        nn->weights[i] = (long double*)malloc(current_size * sizeof(long double));
        nn->biases[i] = (long double*)malloc(layers[i + 1] * sizeof(long double));
        nn->weights_gradients[i] = (long double*)calloc(current_size, sizeof(long double));
        nn->biases_gradients[i] = (long double*)calloc(layers[i + 1], sizeof(long double));
        nn->weights_m[i] = (long double*)calloc(current_size, sizeof(long double));
        nn->weights_v[i] = (long double*)calloc(current_size, sizeof(long double));
        nn->biases_m[i] = (long double*)calloc(layers[i + 1], sizeof(long double));
        nn->biases_v[i] = (long double*)calloc(layers[i + 1], sizeof(long double));
    ...
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        size_t current_size = layers[i] * layers[i + 1];
        long double scale = sqrtl(2.0L / (layers[i] + layers[i + 1]));
...
NN_t* NN_init_random(size_t num_inputs, size_t num_outputs) {
    ...
    // Create random network architecture (2-4 hidden layers)
    size_t num_hidden_layers = 2 + (rand() % 3);  // Random number between 2-4
    ...
    // Set random activation functions (except output layer which uses sigmoid)
    for (size_t i = 0; i < total_layers - 2; i++) {
        act_funcs[i] = rand() % ACTIVATION_TYPE_COUNT;
        act_derivs[i] = act_funcs[i];  // Matching derivative
    }
    // Output layer uses sigmoid for classification
    act_funcs[total_layers - 2] = SIGMOID;
    act_derivs[total_layers - 2] = SIGMOID_DERIVATIVE;
...
void NN_destroy(NN_t* nn) {
    ...
    // Free activation function arrays
    ...
    // Free all layer-specific memory
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        if (nn->weights && nn->weights[i]) {
            free(nn->weights[i]);
        }
        if (nn->biases && nn->biases[i]) {
            free(nn->biases[i]);
        }
        if (nn->weights_gradients && nn->weights_gradients[i]) {
            free(nn->weights_gradients[i]);
        }
        if (nn->biases_gradients && nn->biases_gradients[i]) {
            free(nn->biases_gradients[i]);
        }
        if (nn->weights_m && nn->weights_m[i]) {
            free(nn->weights_m[i]);
        }
        if (nn->weights_v && nn->weights_v[i]) {
            free(nn->weights_v[i]);
        }
        if (nn->biases_m && nn->biases_m[i]) {
            free(nn->biases_m[i]);
        }
        if (nn->biases_v && nn->biases_v[i]) {
            free(nn->biases_v[i]);
        }
    }
    ...
    if (nn->weights_gradients) free(nn->weights_gradients);
    if (nn->biases_gradients) free(nn->biases_gradients);
    if (nn->weights_m) free(nn->weights_m);
    if (nn->weights_v) free(nn->weights_v);
    if (nn->biases_m) free(nn->biases_m);
    if (nn->biases_v) free(nn->biases_v);
...
void sgd(NN_t* nn) {
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        ...
        for (size_t j = 0; j < current_layer * next_layer; j++) {
            nn->weights[i][j] -= nn->learningRate * nn->weights_gradients[i][j];
        }
        
        for (size_t j = 0; j < next_layer; j++) {
            nn->biases[i][j] -= nn->learningRate * nn->biases_gradients[i][j];
        }
    }
}
    ...
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        ...
        for (size_t j = 0; j < current_layer * next_layer; j++) {
            nn->weights_v[i][j] = decay_rate * nn->weights_v[i][j] + 
                                 (1 - decay_rate) * nn->weights_gradients[i][j] * nn->weights_gradients[i][j];
            nn->weights[i][j] -= nn->learningRate * nn->weights_gradients[i][j] / 
        ...
        for (size_t j = 0; j < next_layer; j++) {
            nn->biases_v[i][j] = decay_rate * nn->biases_v[i][j] + 
                                (1 - decay_rate) * nn->biases_gradients[i][j] * nn->biases_gradients[i][j];
            nn->biases[i][j] -= nn->learningRate * nn->biases_gradients[i][j] / 
...
void adagrad(NN_t* nn) {
    ...
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        ...
        for (size_t j = 0; j < current_layer * next_layer; j++) {
            nn->weights_v[i][j] += nn->weights_gradients[i][j] * nn->weights_gradients[i][j];
            nn->weights[i][j] -= nn->learningRate * nn->weights_gradients[i][j] / 
                                (sqrtl(nn->weights_v[i][j]) + epsilon);
        }
        
        for (size_t j = 0; j < next_layer; j++) {
            nn->biases_v[i][j] += nn->biases_gradients[i][j] * nn->biases_gradients[i][j];
            nn->biases[i][j] -= nn->learningRate * nn->biases_gradients[i][j] / 
                               (sqrtl(nn->biases_v[i][j]) + epsilon);
        }
    }
...
void adam(NN_t* nn) {
    ...
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        ...
        for (size_t j = 0; j < current_layer * next_layer; j++) {
            nn->weights_m[i][j] = beta1 * nn->weights_m[i][j] + (1 - beta1) * nn->weights_gradients[i][j];
            nn->weights_v[i][j] = beta2 * nn->weights_v[i][j] + 
                                 (1 - beta2) * nn->weights_gradients[i][j] * nn->weights_gradients[i][j];
            
            long double m_hat = nn->weights_m[i][j] / (1 - powl(beta1, nn->t));
        ...
        for (size_t j = 0; j < next_layer; j++) {
            nn->biases_m[i][j] = beta1 * nn->biases_m[i][j] + (1 - beta1) * nn->biases_gradients[i][j];
            nn->biases_v[i][j] = beta2 * nn->biases_v[i][j] + 
                                (1 - beta2) * nn->biases_gradients[i][j] * nn->biases_gradients[i][j];
            
            long double m_hat = nn->biases_m[i][j] / (1 - powl(beta1, nn->t));
...
void nag(NN_t* nn) {
    ...
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        ...
        for (size_t j = 0; j < current_layer * next_layer; j++) {
            ...
            nn->weights_v[i][j] = momentum * nn->weights_v[i][j] - nn->learningRate * nn->weights_gradients[i][j];
            nn->weights[i][j] += -momentum * prev_velocity + (1 + momentum) * nn->weights_v[i][j];
        }
        
        for (size_t j = 0; j < next_layer; j++) {
            long double prev_velocity = nn->biases_v[i][j];
            nn->biases_v[i][j] = momentum * nn->biases_v[i][j] - nn->learningRate * nn->biases_gradients[i][j];
...
long double* NN_matmul(long double inputs[], long double weights[], long double biases[], 
    ...
    long double* result = (long double*)malloc(output_size * sizeof(long double));
    
    for (size_t i = 0; i < output_size; i++) {
        result[i] = biases[i];
        for (size_t j = 0; j < input_size; j++) {
            result[i] += weights[j * output_size + i] * inputs[j];
        }
    }
...
void NN_backprop(NN_t* nn, long double inputs[], long double y_true, long double y_predicted) {
    ...
    long double error = nn->lossDerivative(y_true, y_predicted);
    ...
    for (size_t i = nn->numLayers - 1; i > 0; i--) {
        ...
        for (size_t j = 0; j < current_layer; j++) {
            ...
            nn->biases_gradients[i - 1][j] = delta;
            
            // Update weight gradients
            for (size_t k = 0; k < prev_layer; k++) {
                nn->weights_gradients[i - 1][k * current_layer + j] = delta * inputs[k];
            }
        }
    }
...
// Helper Functions - Function Getters
ActivationFunction get_activation_function(ActivationFunctionType type) {
    if (type >= ACTIVATION_TYPE_COUNT) return NULL;
...
ActivationDerivativeType string_to_activation_derivative(const char* str) {
    if (!str) return -1;
    if (strcmp(str, "sigmoid_derivative") == 0) return SIGMOID_DERIVATIVE;
    if (strcmp(str, "tanh_derivative") == 0) return TANH_DERIVATIVE;
    if (strcmp(str, "relu_derivative") == 0) return RELU_DERIVATIVE;
    if (strcmp(str, "linear_derivative") == 0) return LINEAR_DERIVATIVE;
    return -1;
}
...
LossDerivativeType string_to_loss_derivative(const char* str) {
    if (!str) return -1;
    if (strcmp(str, "mse_derivative") == 0) return MSE_DERIVATIVE;
    if (strcmp(str, "mae_derivative") == 0) return MAE_DERIVATIVE;
    if (strcmp(str, "huber_derivative") == 0) return HUBER_DERIVATIVE;
    if (strcmp(str, "log_loss_derivative") == 0) return LL_DERIVATIVE;
    if (strcmp(str, "cross_entropy_derivative") == 0) return CE_DERIVATIVE;
    return -1;
}
...
int NN_save(NN_t* nn, const char* filename) {
    ...
    // Write configuration
    fwrite(&nn->regType, sizeof(RegularizationType), 1, file);
    fwrite(&nn->optType, sizeof(OptimizerType), 1, file);
...
NN_t* NN_load(const char* filename) {
    ...
    if (!nn->weights || !nn->biases || !nn->weights_gradients || !nn->biases_gradients ||
        !nn->weights_m || !nn->weights_v || !nn->biases_m || !nn->biases_v ||
    ...
    // Allocate and read layer-specific memory
    for (size_t i = 0; i < nn->numLayers - 1; i++) {
        size_t weights_size = nn->layers[i] * nn->layers[i + 1];
        ...
        nn->weights_gradients[i] = (long double*)calloc(weights_size, sizeof(long double));
        nn->biases_gradients[i] = (long double*)calloc(nn->layers[i + 1], sizeof(long double));
        nn->weights_m[i] = (long double*)calloc(weights_size, sizeof(long double));
        nn->weights_v[i] = (long double*)calloc(weights_size, sizeof(long double));
        nn->biases_m[i] = (long double*)calloc(nn->layers[i + 1], sizeof(long double));
        nn->biases_v[i] = (long double*)calloc(nn->layers[i + 1], sizeof(long double));
        
        // Check allocations
        if (!nn->weights[i] || !nn->biases[i] || !nn->weights_gradients[i] || 
            !nn->biases_gradients[i] || !nn->weights_m[i] || !nn->weights_v[i] || 
            !nn->biases_m[i] || !nn->biases_v[i]) {
            NN_destroy(nn);
            fclose(file);
            return NULL;
        }
    ...
    // Read configuration
    if (fread(&nn->regType, sizeof(RegularizationType), 1, file) != 1 ||
        fread(&nn->optType, sizeof(OptimizerType), 1, file) != 1) {
        NN_destroy(nn);
        fclose(file);
        return NULL;
    }
    
    // Set function pointers based on loaded configuration
    nn->regularization = get_regularization_function(nn->regType);
    nn->optimizer = get_optimizer_function(nn->optType);
    
    // Check if function pointers were set successfully
    if (!nn->regularization || !nn->optimizer) {
        NN_destroy(nn);
        fclose(file);
        return NULL;
    }
...
long double h(long double y_true, long double y_pred) {  // Huber Loss
...
long double h_derivative(long double y_true, long double y_pred) {  // Huber Loss Derivative
    const long double delta = 1.0L;
