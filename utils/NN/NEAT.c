...
extern const LossFunction LOSS_FUNCTIONS[];
extern const LossDerivative LOSS_DERIVATIVES[];
...
Perceptron_t *Perceptron_init_random(unsigned int num_inputs, unsigned int num_outputs) {
    ...
    size_t layers[] = {num_inputs, 4, num_outputs, 0};  // Hidden layer of size 4
    
    // Create activation functions array
    ActivationFunctionType actFuncs[] = {RELU, SIGMOID};
    ActivationDerivativeType actDerivs[] = {RELU_DERIVATIVE, SIGMOID_DERIVATIVE};
    
    printf("Perceptron_init_random: Initializing neural network\n");
    fflush(stdout);
    
    NN_t *nn = NN_init(layers, actFuncs, actDerivs, MSE, MSE_DERIVATIVE, 0.01L, L2, SGD);
    if (!nn) {
...
NEAT_t *NEAT_init(unsigned int num_inputs, unsigned int num_outputs, unsigned int initial_population) {
    printf("NEAT_init: Starting with %u inputs, %u outputs, %u initial nodes\n", 
           num_inputs, num_outputs, initial_population);
    fflush(stdout);
    
    NEAT_t *neat = malloc(sizeof(NEAT_t));
    if (!neat) return NULL;

    neat->num_nodes = initial_population;
    ...
    // Create initial population with specified input/output sizes
...
long double* NEAT_forward(NEAT_t* neat, long double inputs[]) {
    ...
    return perceptron_forward(best, inputs);
}
...
void NEAT_save_best(NEAT_t* neat, const char* filename) {
    if (!neat || !neat->nodes || !filename) return;
    ...
    fwrite(&best->nn->numLayers, sizeof(size_t), 1, file);
    fwrite(best->nn->layers, sizeof(size_t), best->nn->numLayers, file);
    
    // Write weights and biases
    for (size_t i = 0; i < best->nn->numLayers - 1; i++) {
        size_t weights_size = best->nn->layers[i] * best->nn->layers[i + 1];
        fwrite(best->nn->weights[i], sizeof(long double), weights_size, file);
        fwrite(best->nn->biases[i], sizeof(long double), best->nn->layers[i + 1], file);
    }

    fclose(file);
}

// Add these getter functions to NN.c
...
LossDerivativeType get_loss_derivative_type(LossDerivative func) {
    for (int i = 0; i < LOSS_DERIVATIVE_TYPE_COUNT; i++) {
        if (LOSS_DERIVATIVES[i] == func) return i;
    }
    return MSE_DERIVATIVE;  // Default
}
