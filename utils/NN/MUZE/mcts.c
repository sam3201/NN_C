...
/* Expand node: use model.predict on node->latent to fill P and return predicted
 * value (caller can use) */
...
/* Backup with discounted sum:
   Given actions[0..depth-1], rewards[0..depth-1] (each reward r_t is reward
   received when taking actions[t]) and leaf_value v (prediction at leaf),
   compute total_return = sum_{i=0..depth-1} gamma^i * r_i + gamma^depth * v
   Then add total_return to W of the root action and ancestors appropriately.
   We implement: for each ancestor along path, add total_return to
   W[action_at_that_level], increment N, recompute Q.
*/
