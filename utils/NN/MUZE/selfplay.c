...
/* Helper: compute discounted returns for an episode of length T.
   rewards[0..T-1], gamma -> returns z_t = sum_{k=0..T-1-t} gamma^k * rewards[t+k]
   output z_out must be length T
*/
static void compute_discounted_returns(const float *rewards, int T, float gamma, float *z_out) {
    for (int t=0;t<T;t++) {
        float acc = 0.0f;
        float g = 1.0f;
        for (int k=t;k<T;k++) {
...
void selfplay_run(MuModel *model,
                  void *env_state,
                  ...
                  ReplayBuffer *rb)
{
    if (!model || !env_reset || !env_step || !mcts_params || !sp_params || !rb) return;
    ...
    for (int ep=0; ep < sp_params->total_episodes; ep++) {
        ...
        while (!done && step < max_steps) {
            ...
            mp.temperature = sp_params->temperature > 0.0f ? sp_params->temperature : mcts_params->temperature;
    ...
    free(obs_buf); free(pi_buf); free(reward_buf); free(act_buf);
}
